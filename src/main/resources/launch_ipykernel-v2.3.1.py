...
# type: ignore
"""
The source of this file is https://github.com/jupyter/enterprise_gateway/blob/3d128abcbea0cdc07c7b4a1c4ef82cb5b6e2515a/etc/kernel-launchers/python/scripts/launch_ipykernel.py
This is not a Data Mechanics source, so linting is deactivated.
"""
...
def initialize_namespace(namespace, cluster_type="spark"):
    ...
    if cluster_type == "spark":
        ...
        def initialize_spark_session():
            ...
            sql = spark.sql
            try:
                # spark._wrapped does not exist with spark 3.3
        ...
        if port_range_size != 0:
            if port_range_size < min_port_range_size:
                raise RuntimeError(
                    ...
                    " env EG_MIN_PORT_RANGE_SIZE".format(
                        port_range, min_port_range_size
                    )
                )
    except ValueError as ve:
        raise RuntimeError(
            "Port range validation failed for range: '{}'.  Error was: {}".format(
                port_range, ve
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "connection_file", nargs="?", help="Connection file to write connection info"
    )
    parser.add_argument(
    ...
    if (
        ...
        if response_addr:
            ...
            if gateway_socket:  # socket in use, start gateway listener thread
                gateway_listener_process = Process(
                    target=gateway_listener,
                    args=(gateway_socket, os.getpid()),
                )
    ...
    start_ipython(
        locals(), cluster_type=cluster_type, connection_file=connection_file, ip=ip
    )
